            ...
            if (isset($_POST['UPLOAD'])) {
                ...
                if ($file_type == "mp4"OR $file_type=="MP4") {
                    if(move_uploaded_file($_FILES['video']['tmp_name'], $target))
                    {
                        ...
                        extract($_POST);
                    $i=$_SESSION['logid'];
                      $qry="INSERT into videos(category_id,user_id,video_title,video_path,video_desc,v_date,status,no_of_shares,like_count,no_of_comments) values ($category,'$i','$video_title','$target','$video_desc',now(),'active','0','0','0')";
                          $id=insert($qry);
                    }
        }
